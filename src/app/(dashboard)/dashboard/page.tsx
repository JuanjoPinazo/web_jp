    ...
    <div className="max-w-3xl mx-auto space-y-12 pb-24 px-4 md:px-0">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          <div key={i} className="p-6 rounded-3xl bg-surface/40 backdrop-blur-md border border-white/5 flex items-center gap-6 group hover:border-white/10 transition-all">
             <div className={`p-4 rounded-2xl bg-[#0a0a0a] ${stat.color} group-hover:scale-110 transition-transform`}>
               <stat.icon size={24} />
             </div>
             <div>
               ...
               <div className="flex items-baseline gap-2">
                 ...
                 <span className="text-[8px] font-black uppercase text-accent/60 tracking-widest">{stat.status}</span>
               </div>
             </div>
          </div>
        ))}
      ...
      {activePlan?.hotels[0] && (
        <section className="relative group cursor-pointer" onClick={() => setSelectedCard(outcomeCards.find(c => c.actionType === 'hotel'))}>
          <div className="absolute inset-0 bg-accent/20 blur-[100px] opacity-20 group-hover:opacity-40 transition-opacity rounded-[3rem]" />
          <div className="relative p-10 rounded-[3rem] bg-gradient-to-br from-surface to-[#0a0a0a] border border-white/10 shadow-2xl overflow-hidden">
             ...
             <div className="space-y-8 relative z-10">
                <div className="space-y-1">
                  ...
                  <h2 className="text-4xl font-black font-heading text-white tracking-tight">{activePlan.hotels[0].hotel_name}</h2>
                </div>
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
           {[
             ...
             <div key={i} className="p-6 rounded-[2rem] bg-surface border border-white/5 space-y-4">
                <div className={`p-3 rounded-xl bg-[#0a0a0a] w-fit ${item.color}`}>
                  <item.icon size={20} />
                </div>
                <div>
      ...
      <section className="space-y-8">
        ...
        <div className="p-8 md:p-12 rounded-[3.5rem] bg-[#0a0a0a]/50 border border-white/5">
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-3">
            {activePlan.documents.map(doc => (
              <div key={doc.id} className="flex items-center justify-between p-6 rounded-3xl bg-surface border border-white/5 group hover:border-accent/30 transition-all">
                <div className="flex items-center gap-4">
                  <div className="p-3 rounded-xl bg-[#0a0a0a] text-accent">
                    <FileText size={20} />
                  </div>
                ...
                <a 
                  ...
                  className="px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-[9px] font-black uppercase tracking-widest text-white hover:bg-accent hover:text-background transition-all"
                >
                  Descargar
                </a>
              </div>
      ...
      {/* 6. SOPORTE CONCIERGE */}
      <section className="p-10 rounded-[3rem] bg-gradient-to-br from-accent/20 to-[#0a0a0a] border border-accent/20">
         <div className="flex flex-col md:flex-row justify-between items-center gap-8">
            <div className="flex items-center gap-6">
               <div className="w-20 h-20 rounded-[2rem] bg-accent/10 border border-accent/20 p-1 overflow-hidden relative">
                  <img src="/juanjo-support.jpg" alt="Juanjo" className="w-full h-full object-cover rounded-[1.8rem] grayscale hover:grayscale-0 transition-all duration-500" />
               </div>
               <div className="space-y-1">
                  <p className="text-[10px] font-black text-accent uppercase tracking-[0.4em]">Tu Coordinador</p>
                  <h4 className="text-3xl font-black font-heading text-white">Juanjo</h4>
                  <p className="text-xs font-bold text-muted uppercase tracking-widest">Respuesta media &lt; 5 min</p>
               </div>
            </div>
            <div className="flex gap-4 w-full md:w-auto">
               <a href="tel:+34000000000" className="flex-1 md:flex-none px-8 py-4 rounded-2xl bg-white/5 border border-white/10 text-[10px] font-black uppercase tracking-widest text-white text-center hover:bg-white/10 transition-all">Llamar</a>
               <a href="https://wa.me/34000000000" className="flex-1 md:flex-none px-8 py-4 rounded-2xl bg-accent text-background text-[10px] font-black uppercase tracking-widest text-center hover:scale-105 transition-all shadow-xl shadow-accent/20">WhatsApp</a>
            </div>
         </div>
      </section>
      ...
      <footer className="pt-8 pb-12 text-center space-y-6">
         <div className="inline-flex flex-col items-center gap-4">
            ...
            <div className="space-y-1">
               <h3 className="text-2xl font-black font-heading text-white uppercase tracking-tight">Todo bajo control</h3>
               <p className="text-sm font-medium text-muted">No tienes que hacer nada más. Disfruta del evento.</p>
            </div>
         </div>
         <div className="pt-10">
            <p className="text-[9px] font-black text-muted/30 uppercase tracking-[0.5em]">JP Intelligence Concierge Edition</p>
         </div>
      </footer>
