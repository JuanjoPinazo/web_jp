...
export async function POST(request: Request) {
  try {
    ...
    // If the user is already registered, 'invite' fails. We handle the fallback.
    ...
    if (linkError && linkError.message.includes('already been registered')) {
      const { data: retryData, error: retryError } = await getSupabaseAdmin().auth.admin.generateLink({
        type: 'magiclink',
        email,
        options: {
          redirectTo: `${process.env.NEXT_PUBLIC_SITE_URL || 'https://jpintelligence.vercel.app'}/set-password`
    ...
    // 4. Send Custom Email via Resend (Only if requested)
    if (sendInvite) {
      try {
